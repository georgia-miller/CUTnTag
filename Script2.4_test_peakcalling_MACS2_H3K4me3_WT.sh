#!/bin/bash
#SBATCH --job-name=Script2.4_MACS2_H3K4me3_WT_test
#SBATCH --time=48:00:00
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=100G
#SBATCH --output=/scratch/prj/id_hill_sims_wellcda/CUTnTag/logs/Script2.4_MACS2_test_%j.log
#SBATCH --error=/scratch/prj/id_hill_sims_wellcda/CUTnTag/logs/Script2.4_MACS2_test_%j.log
...
echo -e "\n######## Script 2.4 to call peaks with MACS2 for each replicate and merged samples for ${modification} ######## \n"
...
# source conda and mamba so it works in non-interactive shells
...
for base_name in "${conditions[@]}"; do
	...
	# activate the macs2 mamba environment
	mamba activate CUTnTag_macs2_env
	...
	for rep in "${replicates[@]}"; do
		...
		sort -k1,1 -k2,2n ${rep}_macs2_peaks.${peak_type} > ${rep}_macs2.sorted.${peak_type}

		# convert to bigwig for IGV visualisation
		sort -k1,1 -k2,2n ${rep}_macs2_treat_pileup.bdg > ${rep}_macs2_treat_pileup.sorted.bdg

		bedGraphToBigWig ${rep}_macs2_treat_pileup.sorted.bdg \
			${mm10_genome} \
			${rep}_macs2.bw
		...
		rm  ${rep}_macs2_treat_pileup.bdg \
			${rep}_macs2_treat_pileup.sorted.bdg
	done
	...
	sort -k1,1 -k2,2n ${base_name}_macs2_peaks.${peak_type} > ${base_name}_macs2.sorted.${peak_type}

	# convert to bigwig for IGV visualisation
	sort -k1,1 -k2,2n ${base_name}_macs2_treat_pileup.bdg > ${base_name}_macs2_treat_pileup.sorted.bdg

	bedGraphToBigWig ${base_name}_macs2_treat_pileup.sorted.bdg \
		${mm10_genome} \
		${base_name}_macs2.bw

	echo -e "\n [${timestamp}] Finished calling peaks for ${base_name} \n"

	rm  ${base_name}_macs2_treat_pileup.bdg \
		${base_name}_macs2_treat_pileup.sorted.bdg
	...
	peak_files=("${dir_output}/${replicates[0]}/${replicates[0]}_macs2.sorted.${peak_type}" \
				"${dir_output}/${replicates[1]}/${replicates[1]}_macs2.sorted.${peak_type}" \
				"${dir_output}/${replicates[2]}/${replicates[2]}_macs2.sorted.${peak_type}")

	# count peaks between replicates to find overlapping ones
	bedtools multiinter -i ${peak_files[@]} > ${base_name}_macs2.multiinter.bed

	# filter to keep peaks present in at least 2 replicates
	awk '$4>=2 {print $1"\t"$2"\t"$3"\t"$4}' ${base_name}_macs2.multiinter.bed > ${base_name}_macs2.consensus_peaks_raw.bed 

	# merge any overlapping/bookmarked peaks
	bedtools merge -i ${base_name}_macs2.consensus_peaks_raw.bed > ${base_name}_macs2.consensus_peaks.bed 
	...
	rm  ${base_name}_macs2.multiinter.bed \
		${base_name}_macs2.consensus_peaks_raw.bed 
